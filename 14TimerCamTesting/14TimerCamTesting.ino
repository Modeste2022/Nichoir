...
void loop() {
    ...
    // Vérifier si 1 minute s'est écoulée pour envoyer une Photo
    ...
    //Vérifier si le PIR détecte, allumer la LED et envoyer une Photo
    //PIR: SDA G4   et   LED: SCL G13
    int pirState = digitalRead(BOARD_PIRPIN);

    if(pirState == HIGH) {
      Serial.println("Détection de mouvement: Capture de photo et flash");
      digitalWrite(BOARD_LEDPIN, HIGH);    //Led On

      if(captureAndSendPhoto()) {
        photoCounter++;
        Serial.printf("✅ Photo #%d envoyée avec succès\n", photoCounter);
        Serial.println("Prochaine photo dans 1 détection de mouvement...\n");
      } else {
        Serial.println("❌ Échec de l'envoi - Nouvelle tentative dans 1 instant");
      }

    } else {
      digitalWrite(BOARD_LEDPIN, LOW);    //Led Off
    }
